		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'typography':
					...
					if(isset($typography_stored['face'])) {
						...
						global $cs_googlefonts, $cs_simplefonts;
                                                $faces = array_merge($cs_simplefonts, $cs_googlefonts);
				...
				case 'background':
					...
					$repeats = array('repeat'=> esc_attr__('Repeat','voyager'),
									'repeat-x'=> esc_attr__('Repeat-x','voyager'),
									'repeat-y'=>esc_attr__('Repeat-y','voyager'),
									'no-repeat'=>esc_attr__('No repeat','voyager'));
					...
					$positions = array('left'=>esc_attr__('Left','voyager'),
									'right'=>esc_attr__('Right','voyager'),
									'center'=>esc_attr__('Center','voyager'));
					...
					$positions2 = array('top'=>esc_attr__('Top','voyager'),
									'bottom'=>esc_attr__('Bottom','voyager'),
									'center'=>esc_attr__('Center','voyager'));
					...
					$attachments = array('scroll'=>esc_attr__('Scroll','voyager'),
									'fixed'=>esc_attr__('Fixed','voyager'),
									'local'=>esc_attr__('Local','voyager'));
				...
				case 'backup':
					...
					$output .= '<p><strong>'. esc_attr__('Last Backup : ', 'voyager').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'select_google_font':
					...
					$output .= '<select class="select of-input google_font_select" name="'.$value['id'].'" id="'. $value['id'] .'">';
                                        $faces = array_merge($cs_simplefonts, $value['options']);
				...
				case 'demo_import':
					...
					$nonce	 = 	wp_create_nonce ('cstheme_demo_import_nonce');

					$output .= '<div class="import-box">';
					$output .= '<input type="hidden" name="cstheme-demo-import-nonce" value="' . $nonce . '" />';
					$output .= '<p>' . esc_attr__( 'Import the demo content as seen on the live page, containing all Pages, Posts, Menus, etc.. Starting the import process more than once is not recommended. You should use the standard WordPress Importer instead.', 'voyager') . '</p><br>';
					$output .= '<h3 class="heading">' . esc_attr__( 'Before the import', 'voyager') . '</h3>';
					$output .= '<p>' . esc_attr__( 'Make sure all necessary plugins are installed and active otherwise their data will not be imported!', 'voyager') . '</p>';
					
					//	Contact Form 7
					$active = is_plugin_active('contact-form-7/wp-contact-form-7.php');
					$plugin_status = ( $active ) ? '<span class="cstheme-active">' . esc_attr__( 'Active', 'voyager' ) . '</span>' : '<span class="cstheme-inactive">' . esc_attr__( 'Inactive', 'voyager' ) . '</span>';
					$output .= '<p>3. ' . $plugin_status . ' - ' . esc_attr__( 'Contact Form 7', 'voyager') . ' - ' . esc_attr__( ' imports Contact Forms', 'voyager' );
					if ( ! $active ){
						$output .=	' ' . '(';
						$output .='<a href="' . admin_url( 'themes.php?page=tgmpa-install-plugins' ) . '">' . esc_attr__( 'Install', 'voyager' ) . '</a>';
						$output .= '/';
						$output .='<a href="' . admin_url( 'themes.php?page=tgmpa-install-plugins' ) . '">' . esc_attr__( 'Activate', 'voyager' ) . '</a>';
						$output .= ')';
					}
					$output .= '</p>';
					
					//	MailChimp for WordPress Lite
					$active = is_plugin_active('mailchimp-for-wp/mailchimp-for-wp.php');
					$plugin_status = ( $active ) ? '<span class="cstheme-active">' . esc_attr__( 'Active', 'voyager' ) . '</span>' : '<span class="cstheme-inactive">' . esc_attr__( 'Inactive', 'voyager' ) . '</span>';
					$output .= '<p>4. ' . $plugin_status . ' - ' . esc_attr__( 'MailChimp for WordPress Lite', 'voyager') . ' - ' . esc_attr__( ' imports MailChimp form', 'voyager' );
					if ( ! $active ){
						$output .=	' ' . '(';
						$output .='<a href="' . admin_url( 'themes.php?page=tgmpa-install-plugins' ) . '">' . esc_attr__( 'Install', 'voyager' ) . '</a>';
						$output .= '/';
					...
					$output .= '<div class="import-started-message">' .
						'<h3 class="heading">' . esc_attr__( 'Import started...', 'voyager' ) . '</h3>' .
						'<div class="cstheme-preloader-holder"><span class="cstheme-preloader"></span></div>' .
						'<p>' . esc_attr__( 'The import might take up to few minutes. Please, be patient and do not reload the page. You will be notified once the import has finished.', 'voyager' ) . '</p>' .
						'</div>';
					$output .= '<div class="import-error-holder">' .
						'<h3 class="heading">' . esc_attr__( 'Import Error!', 'voyager' ) . '</h3>' .
						'<p>' . esc_attr__( 'An error occurred during the import process.', 'voyager' ) . '</p>' .
						'<div class="import-error-message"></div>' .
						'</div>';
					$output .= '<div class="import-success-holder">' .
						'<h3 class="heading">' . esc_attr__( 'All Done. Have Fun!', 'voyager' ) . '</h3>' .
						'<p>' . esc_attr__( 'Remember to update the passwords and roles of imported users.', 'voyager' ) . '</p>' .
